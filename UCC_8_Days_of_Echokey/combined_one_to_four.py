#!/usr/bin/env python3
"""
8 Days of EchoKey — Days 1–4 Unified Axis Gate (stand‑alone; walkthrough in comments)
CC0‑1.0 — Public Domain

WHAT THIS FILE IS
-----------------
A single, self‑contained Python module unifying Day‑1 (Cyclicity, k=0), Day‑2 (Recursion, k=1),
Day‑3 (Fractality, k=2), and Day‑4 (Diagonality‑XY, k=3) into a generic one‑qubit EchoKey gate with
a layout‑aware rewrite pass.
...
axis–angle unitary. For k=2 (Day‑3) we add a **Z‑axis fast path**: if the resolved axis is ≈ ±Z,
...
6) Z‑fast path (k=2):         If n̂ ≈ ±Z, exp(−i θ n̂·σ) ≍ RZ(sign(n_z)·2θ) (emit a single native gate).
...
Run me:
  $ python echokey_axisgate_1234.py
...
def ek7_A_default() -> np.ndarray:
    ...
    Row indices: 0→X (Day‑1), 1→Y (Day‑2), 2→Z (Day‑3), 3→XY (Day‑4), others are simple diagonals.
    """
    A = np.array([
...
def site_specific_A(num_qubits: int, tilt_a3_every:int=2, tilt_a4_every:int=2) -> Dict[int, np.ndarray]:
    """Attach a distinct A^(p) to each physical wire p.
    By default a3 = Z for all wires (enables Z fast path). To exercise the general ZYZ path,
    tilt a3 slightly off Z on every `tilt_a3_every`‑th wire and tilt a4 off the XY plane on
    every `tilt_a4_every`‑th wire.
    """
...
# =============================================================================
# 3) Layout‑aware rewrite pass: ek_axis(k,θ) → ZYZ, with Z‑fast path for k=2 (or any n̂≈±Z)
# =============================================================================
...
def ex_multiqubit_per_site_axes(n: int = 4) -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(n)
    As = site_specific_A(n, tilt_a3_every=2, tilt_a4_every=2)
    # Mix k across wires (0,1,2,3) and entangle neighbors
    ks = [0,1,2,3][:n]
    for q in range(n):
        k = ks[q]
...
def main():
    print("=== Days 1–4: Unified Axis Gate → ZYZ (Qiskit 1.4) — layout‑aware + Z‑fast ===")

    # Day‑3 Z‑fast path demo
    run_and_check(*ex_day3_demo_fastpath(), show_circuits=True)

    # Day‑1, Day‑2, Day‑4 demos
