#!/usr/bin/env python3
"""
8 Days of EchoKey — Days 1–7 Unified Axis Gate (stand‑alone; walkthrough in comments)
...
Day‑4 (Diagonality‑XY, k=3), Day‑5 (Diagonality‑YZ, k=4), Day‑6 (Diagonality‑XZ, k=5),
Day‑7 (Diagonality‑XYZ body, k=6)
...
Run me:
  $ python combined_one_to_seven.py
"""
...
def ek7_A_default() -> np.ndarray:
    ...
    k:0→X, 1→Y, 2→Z, 3→XY, 4→YZ, 5→XZ, 6→XYZ.
    """
    A = np.array([
        ...
        [ 1, 1, 1],  # k=6 : XYZ body diagonal (Diagonality‑XYZ)
    ], dtype=float)
...
def site_specific_A(num_qubits: int,
                    ...
                    tilt_a5_every:int=2,
                    tilt_a6_every:int=2,
                    tilt_a7_every:int=2) -> Dict[int, np.ndarray]:
    ...
    for p in range(num_qubits):
        ...
        if tilt_a5_every > 0 and (p % tilt_a5_every == 1):
            A[4] = axis_from_components(x=0.20, y=1.0, z=1.0)
        # XZ: nudge toward +Y
        if tilt_a6_every > 0 and (p % tilt_a6_every == 1):
            A[5] = axis_from_components(x=1.0, y=0.20, z=1.0)
        # XYZ: nudge toward +X
        if tilt_a7_every > 0 and (p % tilt_a7_every == 1):
            A[6] = axis_from_components(x=1.10, y=1.0, z=1.0)
        As[p] = A
    return As
...
def ex_day7_demo() -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(1)
    qc.append(EchoKeyAxisGate(6, -0.41), [0])
    qc.append(HGate(), [0])
    weights = {0: SiteWeights(A=ek7_A_default())}
    return ("[Day‑7] 1q ek_axis(k=6) (XYZ body) → H", qc, weights)
...
def ex_multiqubit_per_site_axes(n: int = 7) -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(n)
    As = site_specific_A(n, tilt_a3_every=2, tilt_a4_every=2, tilt_a5_every=2, tilt_a6_every=2, tilt_a7_every=2)
    ks = [0,1,2,3,4,5,6][:n]
    for q in range(n):
        qc.append(EchoKeyAxisGate(ks[q], 0.17*(q+1)), [q])
        if q < n-1:
...
def main():
    print("=== Days 1–7: Unified Axis Gate → ZYZ (Qiskit 1.4) — layout‑aware + Z‑fast ===")
    ...
    run_and_check(*ex_day7_demo(), show_circuits=False)
    ...
    run_and_check(*ex_multiqubit_per_site_axes(7), show_circuits=False)
