#!/usr/bin/env python3
"""
8 Days of EchoKey — Day 7: Diagonality (XYZ body) → ZYZ rewrite, layout‑aware
CC0‑1.0 — Public Domain

WHAT THIS FILE SHOWS (high level)
---------------------------------
1) EchoKey recap: each local generator is E_k^∘ = a_k · σ with a_k ∈ ℝ^3, ||a_k||=1 (rows of A).
2) Day‑7 selects the 7th direction (code index k=6), the XYZ body diagonal: a7 ≈ (1,1,1)/√3.
   Define ek_diagxyz(θ) := exp(−i θ (a7 · σ)).
3) We provide a symbolic gate + a layout‑aware rewrite pass:
      ek_diagxyz(θ)  →  RZ(α) · RY(β) · RZ(γ)
   where (α,β,γ) are from exact SU(2) synthesis of U(2θ, a7). No approximations; fidelity ≈ 1.
...
5) Examples: single‑qubit, sequences, two‑qubit with CX, per‑site frames (tilting a7), random battery.

MATH WALKTHROUGH (concise)
--------------------------
Axis–angle form: U(φ, n̂) = cos(φ/2) I − i sin(φ/2) (n̂·σ), with φ=2θ and n̂=a7.
ZYZ completeness: ∀U∈SU(2), ∃ α,β,γ s.t. U ≍ RZ(α) RY(β) RZ(γ) (≍ up to global phase).
Rewrite (per gate): resolve physical wire p via layout → use A^(p)[6] as a7 → synthesize ZYZ.

Run me:
  $ python day_07_echokey_diagonality_Xyz.py
"""
...
def ek7_A_default() -> np.ndarray:
    """Baseline 7×3 matrix A; rows are unit vectors a_k^T. Row 7 (index 6) is XYZ body diagonal."""
    A = np.array([
        [ 1, 0, 0],  # k=0 : X
        [ 0, 1, 0],  # k=1 : Y
        [ 0, 0, 1],  # k=2 : Z
        ...
        [ 1, 1, 1],  # k=6 : XYZ body diagonal  ← Day 7
    ], dtype=float)
...
def site_specific_A(num_qubits: int, tilt_a7_every:int=2) -> Dict[int, np.ndarray]:
    """Give each wire its own A^(p); optionally tilt the a7 row slightly to exercise the
    general ZYZ path under layout‑aware selection.
    """
    As: Dict[int, np.ndarray] = {}
    base = ek7_A_default()
    for p in range(num_qubits):
        A = base.copy()
        if tilt_a7_every > 0 and (p % tilt_a7_every == 1):
            # start from XYZ (1,1,1), tilt a bit toward +X to move it off the perfect body diagonal
            a7 = axis_from_components(x=1.10, y=1.0, z=1.0)
            A[6] = a7
        As[p] = A
    return As

# ----------------------------------
# 1) Symbolic Day‑7 gate (k=6)
# ----------------------------------

class EchoKeyDiagonalityXYZGate(Gate):
    """Symbolic ek_diagxyz(θ) ≡ exp(−i θ (a7 · σ)), where a7 is row 6 (index 6) of A."""
    def __init__(self, theta: float):
        super().__init__(name="ek_diagxyz", num_qubits=1, params=[float(theta)])
        self.definition = QuantumCircuit(1, name="ek_diagxyz")  # symbolic placeholder

# -------------------------------------------------
# 2) The rewrite pass: ek_diagxyz(θ) → RZ · RY · RZ
...
class EchoKeyDiagonalityXYZRewritePass(TransformationPass):
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for node in list(dag.op_nodes()):
            if isinstance(node.op, EchoKeyDiagonalityXYZGate):
                theta = float(node.op.params[0])
                p = self._physical_index_for_node(dag, node)
                A = self.W.get(p, SiteWeights(ek7_A_default())).A
                a7 = A[6]
                if not np.isfinite(a7).all() or np.linalg.norm(a7) < 1e-15:
                    raise ValueError(f"A[6] invalid/degenerate for qubit {p}")
                nhat = a7 / np.linalg.norm(a7)
...
def materialize_ek_ops(qc: QuantumCircuit, weights: Dict[int, SiteWeights]) -> QuantumCircuit:
    ...
    for inst in qc.data:
        ...
        if isinstance(op, EchoKeyDiagonalityXYZGate):
            theta = float(op.params[0])
            q_index = qc.find_bit(qubits[0]).index
            A = weights.get(q_index, SiteWeights(ek7_A_default())).A
            a7 = A[6] / np.linalg.norm(A[6])
            H = theta * (a7[0]*SX + a7[1]*SY + a7[2]*SZ)
            U = expm(-1j * H)
            out.append(UnitaryGate(U, label="ek_diagxyz(mat)"), [qubits[0]])
        else:
...
def compile_with_pass_only(qc: QuantumCircuit, weights: Dict[int, SiteWeights]) -> QuantumCircuit:
    pm = PassManager([EchoKeyDiagonalityXYZRewritePass(weights)])
    return pm.run(qc)
...
def ex1_simple() -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(1)
    qc.append(EchoKeyDiagonalityXYZGate(0.40), [0])
    qc.append(HGate(), [0])
    weights = {0: SiteWeights(A=ek7_A_default())}
    return ("[Ex1] 1q ek_diagxyz → H", qc, weights)


def ex2_sequence() -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(1)
    qc.append(RXGate(0.11), [0])
    qc.append(EchoKeyDiagonalityXYZGate(-0.42), [0])
    qc.append(RYGate(0.23), [0])
    qc.append(EchoKeyDiagonalityXYZGate(0.80), [0])
    qc.append(RZGate(-0.31), [0])
    weights = {0: SiteWeights(A=ek7_A_default())}
    return ("[Ex2] 1q sequence (RX, ek_diagxyz, RY, ek_diagxyz, RZ)", qc, weights)


def ex3_two_qubit() -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(2)
    qc.append(HGate(), [0])
    qc.append(EchoKeyDiagonalityXYZGate(0.50), [0])
    qc.append(CXGate(), [0, 1])
    qc.append(EchoKeyDiagonalityXYZGate(-0.25), [1])
    qc.append(RYGate(0.40), [1])
    weights = {0: SiteWeights(ek7_A_default()), 1: SiteWeights(ek7_A_default())}
    return ("[Ex3] 2q CX between ek_diagxyz ops", qc, weights)


def ex4_multiqubit_mixed_axes(n: int = 5) -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(n)
    As = site_specific_A(n, tilt_a7_every=2)
    for q in range(n):
        qc.append(EchoKeyDiagonalityXYZGate(0.17*(q+1)), [q])
        if q < n-1:
            qc.append(CXGate(), [q, q+1])
    weights = {q: SiteWeights(A=As[q]) for q in range(n)}
    return (f"[Ex4] {n}q per‑site a7 (some tilted) + CX chain", qc, weights)


def ex5_random_battery(m: int = 8, seed: int = 47) -> List[Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]]:
    ...
    for i in range(m):
        ...
        for _ in range(L):
            ...
            if sel == 0:
                q = int(rng.integers(0, n)); qc.append(EchoKeyDiagonalityXYZGate(float(rng.normal()*0.7)), [q])
            elif sel == 1:
...
def main():
    print("=== Day 7: Diagonality (XYZ body) — ZYZ rewrite, layout‑aware (Qiskit 1.4) ===")
    ...
    run_and_check(*ex4_multiqubit_mixed_axes(5), show_circuits=False)
    for label, qc, weights in ex5_random_battery(m=8, seed=47):
