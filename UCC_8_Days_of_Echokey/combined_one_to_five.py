#!/usr/bin/env python3
"""
8 Days of EchoKey — Days 1–5 Unified Axis Gate (stand‑alone; walkthrough in comments)
CC0‑1.0 — Public Domain

WHAT THIS FILE IS
-----------------
A single, self‑contained Python module unifying Day‑1 (Cyclicity, k=0), Day‑2 (Recursion, k=1),
Day‑3 (Fractality, k=2), Day‑4 (Diagonality‑XY, k=3), and Day‑5 (Diagonality‑YZ, k=4)
...
6) Z‑fast path:               If n̂ ≈ ±Z, exp(−i θ n̂·σ) ≍ RZ(sign(n_z)·2θ) (emit a single native gate).
...
Run me:
  $ python echokey_axisgate_12345.py
...
def ek7_A_default() -> np.ndarray:
    ...
    Row indices: 0→X (Day‑1), 1→Y (Day‑2), 2→Z (Day‑3), 3→XY (Day‑4), 4→YZ (Day‑5),
    others are simple diagonals.
    """
    A = np.array([
...
def site_specific_A(num_qubits: int,
                    tilt_a3_every:int=2,
                    tilt_a4_every:int=2,
                    tilt_a5_every:int=2) -> Dict[int, np.ndarray]:
    ...
    tilt a3 slightly off Z on every `tilt_a3_every`‑th wire, tilt a4 off the XY plane on
    every `tilt_a4_every`‑th wire, and tilt a5 slightly toward +X on every `tilt_a5_every`‑th wire.
    """
...
def ex_multiqubit_per_site_axes(n: int = 5) -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(n)
    As = site_specific_A(n, tilt_a3_every=2, tilt_a4_every=2, tilt_a5_every=2)
    # Mix k across wires (0..4) and entangle neighbors
    ks = [0,1,2,3,4][:n]
    for q in range(n):
        k = ks[q]
...
def main():
    print("=== Days 1–5: Unified Axis Gate → ZYZ (Qiskit 1.4) — layout‑aware + Z‑fast ===")

    # Day‑3 Z‑fast path demo
    run_and_check(*ex_day3_demo_fastpath(), show_circuits=True)

    # Day‑1, Day‑2, Day‑4, Day‑5 demos
    ...
    run_and_check(*ex_multiqubit_per_site_axes(5), show_circuits=False)
