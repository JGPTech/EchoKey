#!/usr/bin/env python3
"""
8 Days of EchoKey — Days 1+2+3 Unified Axis Gate (stand‑alone; walkthrough in comments)
CC0‑1.0 — Public Domain

WHAT THIS FILE IS
-----------------
A single, self‑contained Python module unifying Day‑1 (Cyclicity, k=0), Day‑2 (Recursion, k=1),
and Day‑3 (Fractality, k=2) into a generic one‑qubit EchoKey gate with a layout‑aware rewrite pass.
...
axis–angle unitary. For Day‑3 (k=2) we add a **Z‑axis fast path**: if the resolved axis is ≈ ±Z,
we emit a single RZ(±2θ). The per‑wire axis a_k comes from a local 7×3 frame A^(p) (one per
...
6) Z‑fast path (Day‑3):       If n̂ ≈ ±Z, then exp(−i θ n̂·σ) ≍ RZ(sign(n_z)·2θ). Use tolerance ε.
7) Layout awareness:          If logical qubit q maps to physical p, use A^(p) to read a_k.
8) SU(2) closure:             [a·σ, b·σ] = 2i (a×b)·σ; with two non‑colinear axes we generate su(2).
...
Run me:
  $ python combined_one_to_three.py
...
def ek7_A_default() -> np.ndarray:
    ...
    Row indices: 0→X (Day‑1), 1→Y (Day‑2), 2→Z (Day‑3), others simple diagonals.
    """
    A = np.array([
...
def site_specific_A(num_qubits: int, tilt_a3_every:int=2) -> Dict[int, np.ndarray]:
    """Attach a distinct A^(p) to each physical wire p.
    By default a3 = Z for all wires (enables Z fast path). To also exercise the general ZYZ path,
    tilt a3 slightly off Z on every `tilt_a3_every`‑th wire.
    """
...
# =============================================================================
# 3) Layout‑aware rewrite pass: ek_axis(k,θ) → ZYZ, with Z‑fast path
# =============================================================================
...
def ex_multiqubit_per_site_axes(n: int = 4) -> Tuple[str, QuantumCircuit, Dict[int, SiteWeights]]:
    qc = QuantumCircuit(n)
    As = site_specific_A(n, tilt_a3_every=2)
    # Mix k across wires (0,1,2) and entangle neighbors
    ks = [0,1,2,0][:n]
    for q in range(n):
        k = ks[q]
...
def main():
    print("=== Days 1+2+3: Unified Axis Gate → ZYZ (Qiskit 1.4) — layout‑aware + Z‑fast ===")

    # Day‑3 Z‑fast path demo
    run_and_check(*ex_day3_demo_fastpath(), show_circuits=True)

    # Day‑1 and Day‑2 demos
